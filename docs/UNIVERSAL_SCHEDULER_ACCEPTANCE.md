...
| `one_due_reentry_event_container_rep` / `one_due_reentry_event_family_rel_closed` with the container chain (`raw_remove_concrete_heap_container_at_removed`, the branch-cased Event-removal lemmas, the membership-free insert container frame) and `one_due_task_event_item_not_in_after`; QAD-false runs `20260804Tone-due-reentry-gateH-17` through `-32` | The container premise is discharged: **the packaged event family relation holds at the insert heap with no premises**.  Every live task's Event-item container survives all three writes -- the due task's own item driven to NULL at its removal and framed through the insertion, survivors by the payload effect, non-members by the sibling item frames -- and the faithful NULL/members equivalence and per-root membership/container iff transport, the due item's unique base membership by pairwise ring disjointness, the root cast non-null from the layout guard.  Divergence post-mortem recorded: a bare field-lvalue pointer equation fed to simp diverges in the pointer-arithmetic normaliser (lift through `arg_cong` to the decoder level); pointer `metis` replaced by the `arg_cong`/`ptr_val` chain. | The delay-owner/insert-geometry equations, the scalar/root equations and the storage-disjointness pack at the new families, then the full gate re-entry assembly, the while-loop induction and the wrapper.  Still zero refinement rungs; the **0/5** score stands. |
| `one_due_reentry_generic_keys`, `one_due_reentry_insert_geometry` (with `raw_sentinel_ptr_val_in_list_region`), `one_due_reentry_delay_owner` and `one_due_reentry_storage_disjoint`; QAD-false runs `20260804Tone-due-reentry-gateH-33` through `-36` | Four more gate clauses at the re-entry families: every live task's **Generic key** survives all three writes and equals the snapshot payload (the moved node by its own key frame, survivors by the payload effect, non-members by sibling frames, the whole Generic side across the Event removal by a uniform item transport); the **insert geometry** holds for any live task's Generic item (guard through the observation and the ABI retyping, sentinel distinctness through the field-span-in-list-region lemma against the list/tcb separation, member regions by managed distinctness); any surviving member of the shrunk delayed ring is **delay-owned** exactly by the delayed root (unique membership by pairwise disjointness, container from the carried source view); and the re-entry generic storages stay **pairwise separated** from the after-removal event storages (the grown target adds only the moved region, which lives in the source storage). | Only the scalar/root equations remain (pinned by the strengthened tail-insert continuation) before the full gate re-entry assembly; then the while-loop induction and the wrapper.  Still zero refinement rungs; the **0/5** score stands. |
| `one_due_gateH_reentry`; QAD-false run `20260804Tone-due-reentry-gateH-38` | **The tick-body gate re-entry is assembled**: a state pinned by the tail-insert continuation (exact insert heap, untouched delayed pointer, raised-or-kept top word, unchanged tick) satisfies the **full gate entry relation** again -- context advanced to the decoded next head with the entry top at the abstract max, the completed five-phase snapshot, the re-entry raw families (insert-end over after-remove on the Generic side, the after-removal Event side) and the unchanged abstract state.  Together with the tail-insert composition, the delayed-remainder execution and the head decode, **one completed tick-body iteration is a gate-to-gate relation-preserving step**, mirroring the resume drain's architecture.  The pure-layer premises (head shape, liveness, dueness, target membership, branch witness, raw head membership) are the loop layer's obligations. | The while-loop composition itself: the loop-body prefix (guards and the due-compare throw), the end-to-end body theorem, the ring-prefix induction with the not-due/NULL exits, the unlocked-source prefix (tick increment, overflow swap, first head read) and the suspended-condition wrapper -- i.e. the complete `vTaskIncrementTick'` summary that discharges the missed-loop and outer-wrapper premises.  Still zero refinement rungs; the **0/5** score stands. |
